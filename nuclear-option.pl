...
# ***********************  WARNING  *******************************
#
#	The sole purpose of this script is to delete *ALL* of the notes
#	in your Simplenote account.
#
#	ALL of them...
#
# *****************************************************************
...
sub deleteNoteOnline {
	...
	if ($allow_server_updates) {
		...
		my $response = $ua->get($url . "delete?key=$key&auth=$token&email=$email&dead=1");
		return $response->content;
	} else {
		return "";
	}
...
sub deleteAllNotesOnServer {
	...
	my $counter = 1; 
	foreach my $key (sort keys %note) {
		print "deleting $counter of $count\n";
		deleteNoteOnline($key);
		$counter++;
	}
}
